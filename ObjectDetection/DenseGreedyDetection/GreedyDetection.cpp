        ...
        for (int ii = firstScaledRow; ii < lastScaledRow; ii ++)
        {
            ...
            for (int jj = firstScaledCol; jj < lastScaledCol; jj ++)
            {
                ...
                if (prob > m_fProbVoteThreshold)
                {
                    ...
                    float cur_cost = std::min(0.0f, costForPoint-in_fPatchGivenCost);
                    float prev_cost = std::min(0.0f, costForPoint-prevGivenCost);
...
void CGreedyDetection::updateAccumulatorScale(int iScale, bool init)
{
    ...
    else
    {
        ...
        firstRow = std::max(0, cvRound(ConvertToScale0(firstScaledRow, lastScale)));
        firstCol = std::max(0, cvRound(ConvertToScale0(firstScaledCol, lastScale)));
        lastRow = std::min(height, cvRound(ConvertToScale0(lastScaledRow, lastScale)));
        lastCol = std::min(width, cvRound(ConvertToScale0(lastScaledCol, lastScale)));
    }
...
void CGreedyDetection::updateGivenCosts()
{
    ...
    int firstRow = std::max(0, cvRound(ConvertToScale0(firstScaledRow, lastScale)));
    int firstCol = std::max(0, cvRound(ConvertToScale0(firstScaledCol, lastScale)));
    int lastRow = std::min(height-1, cvRound(ConvertToScale0(lastScaledRow, lastScale)));
    int lastCol = std::min(width-1, cvRound(ConvertToScale0(lastScaledCol, lastScale)));
...
void CGreedyDetection::drawDetections(const char *in_sPath)
{
    ...
    for (unsigned int iDetection = 0; iDetection < m_vecDetections.size(); iDetection++)
    {
        ...
        CvPoint pt1, pt2;
        pt1.x = cvRound( std::max(0.0f, ConvertToScale0(curDetJ - curBBoxWidth/2.0f, curDetScale)) );
        pt1.y = cvRound( std::max(0.0f, ConvertToScale0(curDetI - curBBoxHeight/2.0f, curDetScale)) );
        pt2.x = cvRound( std::min<float>(width-1, ConvertToScale0(curDetJ + curBBoxWidth/2.0f, curDetScale)) );
        pt2.y = cvRound( std::min<float>(height-1, ConvertToScale0(curDetI + curBBoxHeight/2.0f, curDetScale)) );
